...
static bool __test_001__(void) {
  ...
  ASSERT_NUM_EQUAL(str_pop_char(&s), -1, "%d");
...
static bool __test_002__(void) {
  ...
  ASSERT_NUM_EQUAL(str_pop_char(&s), (int)'c', "%d");
...
static bool __test_003__(void) {
  string s = {0};
  (void)str_init(&s, k_alloc, 16);
  assert(str_push_cstr(&s, 1000, NULL));
...
static bool __test_004__(void) {
  string s = {0};
  (void)str_init(&s, k_alloc, 16);
  assert(str_push_cstr(&s, 3, (const unsigned char *)"truncated"));
...
static bool __test_005__(void) {
  ...
  (void)str_init(&s, k_alloc, 16);
  assert(str_push_cstr(&s, sizeof(src), src));
  ASSERT_NUM_EQUAL(str_length(&s), (size_t)2, "%zu");
  ASSERT_STR_EQUAL((char *)s._vec._ptr, "ab");
  str_deinit(&s);
  return (true);
...
static bool __test_006__(void) {
  ...
  assert(str_push_cstr(&a, 1000, (const unsigned char *)"foo"));
  assert(str_push_cstr(&b, 1000, (const unsigned char *)"bar"));
  assert(str_append(&a, &b));
...
static bool __test_007__(void) {
  string s = {0};
  (void)str_init(&s, k_alloc, 16);
  assert(str_push_cstr(&s, 1000, (const unsigned char *)"ab"));
  assert(str_append(&s, &s));
...
static bool __test_008__(void) {
  string s = {0};
  (void)str_init(&s, k_alloc, 16);
  assert(str_push_cstr(&s, 1000, (const unsigned char *)"id="));
  assert(str_append_format(&s, 64, "%d", 7));
...
static bool __test_009__(void) {
  string s = {0};
  (void)str_init(&s, k_alloc, 16);
  assert(str_pushf_char(&s, 'c'));
  assert(str_pushf_char(&s, 'b'));
  assert(str_pushf_char(&s, 'a'));
...
static bool __test_010__(void) {
  string s = {0};
  (void)str_init(&s, k_alloc, 16);
  ASSERT_NUM_EQUAL(str_popf_char(&s), -1, "%d");
  assert(str_push_cstr(&s, 1000, (const unsigned char *)"abc"));
  ASSERT_NUM_EQUAL(str_popf_char(&s), (int)'a', "%d");
...
static bool __test_011__(void) {
  string s = {0};
  (void)str_init(&s, k_alloc, 8);
  assert(str_push_cstr(&s, 1000, (const unsigned char *)"["));
  assert(str_append_format(&s, 32, "%s", "INFO"));
  assert(str_push_cstr(&s, 1000, (const unsigned char *)"] "));
  assert(str_append_format(&s, 32, "code=%d", 200));
...
static bool __test_012__(void) {
  ...
  (void)str_init(&s, k_alloc, 16);
  for (int i = 0; i < 32; i++) {
    assert(str_push_char(&s, (unsigned char)('a' + (i % 26))));
  }
  while (str_pop_char(&s) >= 0) {
    /* drain */
  }
  ASSERT_NUM_EQUAL(str_length(&s), (size_t)0, "%zu");
  ASSERT_NUM_EQUAL(s._vec._len, (size_t)1, "%zu");
...
TEST_FUNCTION void str_endops_specs(void) {
  __test_start__;
